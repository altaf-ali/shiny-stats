...
ols_sidebar <- function(id) {
  ns <- NS(id)

  list(
    sliderInput(ns('sample_size'), 'Sample Size',
                min = 2,
                max = nrow(CASchools),
                value = 10,
                step = 5,
    ...
    div(
      class = "sidebar-well",
      actionButton(ns("resample"), label = "Resample", icon = icon("refresh"))
    ),

    sliderInput(ns('confidence_interval'), 'Confidence Interval (%)',
                min = 0,
                max = 99.9,
                value = 95,
                step = 0.1),

    checkboxGroupInput(ns("show_residuals"), label = "Show Residuals",
      ...
      column(width = 5,
             ...
             fluidRow(
               column(width = 8,
                      ...
                      ggvisOutput(ns('tdist_plot'))
               ),
               column(width = 4,
                      align = "right",
                      tableOutput(ns("residuals"))
               )
             ),
             ggvisOutput(ns('pvalue_plot'))
      )
    )
  )
}

ols_server <- function(input, output, session, id, data) {
  ns <- NS(id)
